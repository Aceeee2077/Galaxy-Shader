...
PROGRAMS = {
    ...
    'gbuffers_weather': ('geometry', 'translucent', []),
...
PROFILE_KEYS = ['shadowMapResolution','shadowDistance','SHADOW_QUALITY','CLOUD_QUALITY',
                'WATER_QUALITY','SSR_QUALITY','AO_QUALITY','INDIRECT_QUALITY',
                'VOLUMETRIC_QUALITY','BLOOM_QUALITY','DOF_QUALITY','MOTION_BLUR']
PROFILES = {
    'POTATO': [1024,'64.0',1,1,1,0,0,0,0,1,0,0],
    'LOW': [1024,'96.0',1,1,1,0,1,0,0,1,0,0],
    'MEDIUM': [2048,'96.0',2,2,2,1,1,0,1,2,0,0],
    'HIGH': [2048,'128.0',3,3,2,2,2,1,2,2,0,0],
    'ULTRA': [4096,'192.0',4,4,3,3,3,2,3,3,0,0],
    'CINEMATIC': [8192,'256.0',4,4,3,4,4,2,4,3,3,0],
}
SCREENS = {
    'LIGHTING': 'SUN_INTENSITY NIGHT_BRIGHTNESS AO_QUALITY INDIRECT_QUALITY VOLUMETRIC_QUALITY',
    'SHADOWS': 'shadowMapResolution shadowDistance SHADOW_QUALITY',
    'ATMOSPHERE': 'FOG_DENSITY WIND_STRENGTH NIGHT_BRIGHTNESS',
    'CLOUDS': 'CLOUD_QUALITY CLOUD_COVERAGE CLOUD_SPEED',
    'END_SKY': 'END_PLANETS END_ORBIT_SPEED',
    'WATER': 'WATER_QUALITY WATER_WAVES SSR_QUALITY',
    'REFLECTIONS': 'SSR_QUALITY WET_SURFACES',
    'MATERIALS': 'PBR_MODE WET_SURFACES',
    'POST_PROCESS': 'BLOOM_QUALITY EXPOSURE AUTO_EXPOSURE SATURATION CONTRAST DOF_QUALITY MOTION_BLUR AA_QUALITY',
    'PERFORMANCE': 'shadowMapResolution shadowDistance CLOUD_QUALITY SSR_QUALITY AO_QUALITY INDIRECT_QUALITY VOLUMETRIC_QUALITY',
}
...
def generate():
    ...
    props = [
        ...
        'screen=<profile> [LIGHTING] [SHADOWS] [ATMOSPHERE] [CLOUDS] [END_SKY] [WATER] [REFLECTIONS] [MATERIALS] [POST_PROCESS] [PERFORMANCE]',
        'screen.columns=2',
        'sliders=CLOUD_COVERAGE CLOUD_SPEED WATER_WAVES EXPOSURE SATURATION CONTRAST FOG_DENSITY WIND_STRENGTH SUN_INTENSITY NIGHT_BRIGHTNESS',
    ]
    ...
    labels = {
        ...
        'SHADOW_QUALITY':('Shadow softness','阴影柔和度'),
        ...
        'MOTION_BLUR':('Motion blur','动态模糊'), 'AA_QUALITY':('Anti-aliasing','抗锯齿'),
    ...
    zh_screens=['光照','阴影','大气','云层','末地星空','水体','反射','材质','后期处理','性能']
    for lang,index in [('en_us',0),('zh_cn',1)]:
        ...
        for i,key in enumerate(SCREENS):
            lines.append(f'screen.{key}={key.replace("_"," ").title() if index==0 else zh_screens[i]}')
        quality=['Off','Low','Medium','High','Ultra'] if index==0 else ['关闭','低','中','高','极高']
        for option in ['CLOUD_QUALITY','SSR_QUALITY','AO_QUALITY','VOLUMETRIC_QUALITY','BLOOM_QUALITY','DOF_QUALITY','MOTION_BLUR']:
